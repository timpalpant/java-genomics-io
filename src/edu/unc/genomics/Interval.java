...
public class Interval implements Serializable {
   ...
   * 
   * @param s
   *          an interval String to parse
   * @return an Interval object parsed from s
   * @throws IntervalException
   *           if s is not a valid interval string
   */
  ...
  public String toGFF() {
    ...
    return chr + "\tSpotArray\tfeature\t" + low() + "\t" + high() + "\t.\t" + strand() + "\t.\tprobe_id=" + idStr
        + ";count=1";
  }

  /**
   * Return this Interval as an entry in a file This method should be overridden
   * by subclasses to produce filetype-specific formats
   * 
  ...
  /**
   * The center of this interval, equal to (start+stop)/2 If the interval does
   * not have a perfect center (even length intervals) then the center is
   * rounded down (floor)
   * 
  ...
  /**
   * If this interval includes the given base pair (assumes that it is on the
   * correct chromosome)
   * 
   * @param bp
   * @return
  ...
  /**
   * Calculate the intersection of this interval and another interval Returns
   * null if the chromosome of other is different The returned interval is
   * always Watson oriented (start <= stop)
   * 
   * @param other
   *          an Interval to intersect with this one
  ...
  /**
   * Calculate the union of this interval and another interval Union is defined
   * as an interval which covers both intervals The returned interval is always
   * Watson oriented (start <= stop) An error is raised if the other interval
   * does not have the same chromosome
   * 
   * @param other
   *          an interval to intersect with this one
  ...
  public final Interval union(final Interval other) throws IntervalException {
    if (other == null) {
      ...
      throw new IntervalException("Cannot union intervals with different chromosomes" + " (" + chr + ", " + other.chr
          + ")");
    }

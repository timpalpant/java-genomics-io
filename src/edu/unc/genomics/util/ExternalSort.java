 ...
 * flag: java com/google/code/externalsorting/ExternalSort somefile.txt out.txt -t 3
 * 
 * For very large files, you might want to use an appropriate flag to allocate
 * more memory to the Java VM: java -Xms2G com/google/code/externalsorting/ExternalSort somefile.txt out.txt
 * 
...
public class ExternalSort {
		...
		try {
			...
			try {
				while (line != null) {
					...
					while ((currentblocksize < blocksize) && ((line = fbr.readLine()) != null)) { // as long as you have enough memory
						tmplist.add(line);
	 ...
	 */
	public static int mergeSortedFiles(List<Path> files, Path outputFile, final Comparator<String> cmp) throws IOException {
